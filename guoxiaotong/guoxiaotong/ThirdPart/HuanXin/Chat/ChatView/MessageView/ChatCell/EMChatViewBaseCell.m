...
//    通过userName获取数据；
    ...
    BasicService *service = [[BasicService alloc] initWithView:nil];
    [service getUserInfo:_messageModel.username callBack:^(BOOL isSuccess, UserInfoModel *userInfo) {
        if (isSuccess) {
            weakSelf.messageModel.headImageURL = [NSURL URLWithString:[NSString stringWithFormat:@"%@%@", API_ROOT_IMAGE_URL, userInfo.picPath]];
            UIImage *placeholderImage = [UIImage imageNamed:@"default_user_icon"];
            [weakSelf.headImageView sd_setImageWithURL:_messageModel.headImageURL placeholderImage:placeholderImage];
        }
    }];
#warning 修改部分--
