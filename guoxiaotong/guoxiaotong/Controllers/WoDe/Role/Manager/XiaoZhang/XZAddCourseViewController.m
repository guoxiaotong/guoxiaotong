...
- (void)setUI {
    ...
    _courseIconButton.frame = CGRectMake(CGRectGetMaxX(iconLabel.frame), 10, 40, 40);
    _courseIconButton.backgroundColor = [UIColor redColor];
    [_courseIconButton setImage:[UIImage imageNamed:@"icon_course_001"] forState:UIControlStateNormal];
    _iconId = 1;
    ...
    UILabel *border = [[UILabel alloc] initWithFrame:CGRectMake(0, CGRectGetMaxY(iconLabel.frame)+1, WIDTH, 1)];
    ...
    _iconsBackView = [[UIView alloc] initWithFrame:CGRectMake(10, CGRectGetMaxY(iconLabel.frame)+5, WIDTH-20, 60*(_num_y+1))];
//    _iconsBackView.backgroundColor = [UIColor blackColor];
...
- (void)setButtons {
    for (NSInteger index = 0; index < 16; index++) {
        ...
        button.frame = CGRectMake(10+(20+_wid)*(index%_num_x), 10+(20+_wid)*(index/_num_x), _wid, _wid);
        button.backgroundColor = [UIColor redColor];
        button.tag = 101+index;
        [button setImage:[UIImage imageNamed:[NSString stringWithFormat:@"icon_course_%ld", index]] forState:UIControlStateNormal];
...
//        self.iconsBackView.frame = CGRectMake(self.iconsBackView.frame.origin.x, self.iconsBackView.frame.origin.y, self.iconsBackView.frame.size.width, 60*_num_y);
    }else {
        self.iconsBackView.hidden = YES;
        self.bottomBackView.frame = CGRectMake(0, 45, WIDTH, 200);
//        self.iconsBackView.frame = CGRectMake(self.iconsBackView.frame.origin.x, self.iconsBackView.frame.origin.y, self.iconsBackView.frame.size.width, 0);
    }
}
